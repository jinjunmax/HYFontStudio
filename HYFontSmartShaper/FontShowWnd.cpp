...
BEGIN_MESSAGE_MAP(CFontShowWnd, CWnd)
	...
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	ON_MESSAGE(MSG_SET_VARATION_WND,OnSetVaration)
END_MESSAGE_MAP()
...
LRESULT	CFontShowWnd::OnSetVaration(WPARAM waram, LPARAM lparam)
{		
	//TraceVariationInf();

	return HY_NOERROR;

}	//end of LRESULT	CFontShowWnd::OnSetVaration()
...
void CFontShowWnd::SelectVariationData(int iInstancID)
{
	if(m_pHYViewCodec)
	{
		/*
		std::vector<short> vtAxis;
		for (int i=0; i<m_pHYViewCodec->m_HYfvar.axisCount; i++)
		{
			vtAxis.push_back(m_pHYViewCodec->m_HYfvar.vtaxes[i]);
		}

		std::vector<short> vtInstacneCoord;
		if ( iInstancID < m_pHYViewCodec->m_HYfvar.vtinstances.size() )
		{
			CHYFvarInstncRecord&  InstacneRecord = m_pHYViewCodec->m_HYfvar.vtinstances[iInstancID];
			for (size_t j=0; j<vtAxis.size(); j++)
			{
				vtInstacneCoord.push_back(InstacneRecord.coordinates[j]);
			}
		}

		std::vector<float> vtInstacneNormCoord;
		if ( iInstancID < m_pHYViewCodec->m_HYfvar.vtinstances.size() )
		{			
			for (size_t j=0; j<vtAxis.size(); j++)
			{
				float f = (float)vtInstacneCoord[j]/vtAxis[j];
				if (f < -1.0) 	f = -1.0;			
				if (f > 1.0) f = 1.0;
				vtInstacneNormCoord.push_back(f);
			}
		}
...
}	// end of void CFontShowWnd::SelectVariationData()

void CFontShowWnd::TraceVariationInf()
{
	...
	{	
		...
		TRACE("axis\n");
		for (int j=0; j<m_pHYViewCodec->m_HYfvar.axisCount; j++)
		{	
			if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_WGHT)
			{
				TRACE("Axis tag=wght, min=%f, default=%f, max=%f\n",
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].minValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].defaultValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].maxValue)
					);
			}

			if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_WDTH)
			{
				TRACE("Axis tag=wdth, min=%f, default=%f, max=%f\n",
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].minValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].defaultValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].maxValue)					
					);
			...
			if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_SLNT)
			{
				TRACE("Axis tag=slnt, min=%f, default=%f, max=%f\n",
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].minValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].defaultValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].maxValue)
					);
			...
			if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_OPSZ)
			{
				TRACE("Axis tag=opsz, min=%f, default=%f, max=%f\n",
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].minValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].defaultValue),
					HY_HYFIXED_to_float(m_pHYViewCodec->m_HYfvar.vtaxes[j].maxValue)
					);
		...
		TRACE("instance\n");
		for (int i=0; i<m_pHYViewCodec->m_HYfvar.instanceCount; i++)
		{
			CHYFvarInstncRecord&  InstacneRecor = m_pHYViewCodec->m_HYfvar.vtinstances[i];
			std::string  SubfamilyName = m_pHYViewCodec->m_HYName.GetNamestring(InstacneRecor.subfamilyNameID);

			TRACE("subfamiyName=%s (",SubfamilyName.c_str()); 
			for ( int j=0; j<m_pHYViewCodec->m_HYfvar.axisCount;j++)
			{
				float fcnd = HY_HYFIXED_to_float(InstacneRecor.coordinates[j]);
				TRACE(" %f ",fcnd); 
			}
			TRACE(")\n");
		}

		TRACE("end of instance\n");
		TRACE("end of fvar\n");
		
		
		TRACE("sharedTuple\n");
		for (int i=0; i<m_pHYViewCodec->m_HYgvar.sharedTupleCount; i++)
		{
			CHYTuple tpl = m_pHYViewCodec->m_HYgvar.sharedTuples[i];
			for (int j=0; j<m_pHYViewCodec->m_HYfvar.axisCount; j++)
			{		
				float fnrmCoordinates = HY_F2DOT14_to_float(tpl.coordinates[j]);

				if (fnrmCoordinates < -1.0)
				{
					fnrmCoordinates = -1.0;
				}
				if (fnrmCoordinates > 1.0)
				...
				if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_WGHT)
				{
					TRACE("Axis tag=wght, normalized coordinates=%f ", fnrmCoordinates);
				}

				if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_WDTH)
				{
					TRACE("Axis tag=wdth, normalized coordinates=%f ", fnrmCoordinates);
				}

				if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_SLNT)
				{
					TRACE("Axis tag=slnt, normalized coordinates=%f ", fnrmCoordinates);
				}

				if (m_pHYViewCodec->m_HYfvar.vtaxes[j].Tag == AXIS_TAG_OPSZ)
				{
					TRACE("Axis tag=opsz, normalized coordinates=%f ", fnrmCoordinates);
				}
			}
			TRACE("\n");
		}
		TRACE("end of sharedTuple\n\n");

		// gVar
		TRACE("GlyphVariationData\n");
		if (m_iGlyphID<m_pHYViewCodec->m_HYgvar.glyphCount)
		{
			glyphVariationData VtData = m_pHYViewCodec->m_HYgvar.vtGlyphVariationData[m_iGlyphID];
			for (int i=0; i<VtData.tupleVariationCount; i++)
			{
				TupleVariationHeader& vtHead = VtData.vttupleVariationHeaders[i];

				int iShardIndex = vtHead.iSharedTupleInx;
				TRACE("the shared tuple records index = %d \n", iShardIndex);

				CHYTuple tpl = m_pHYViewCodec->m_HYgvar.sharedTuples[iShardIndex];
				for (int j=0; j<m_pHYViewCodec->m_HYfvar.axisCount; j++)
				{		
					float fnrmCoordinates = HY_F2DOT14_to_float(tpl.coordinates[j]);
					if (fnrmCoordinates < -1.0)
				...
				TRACE("\n");
				for (int j=0; j<m_pGlyph->GetTotalPointsNumber()+4; j++)
				{
					 short x = vtHead.vtVariationPoint_X[j];
					 short y = vtHead.vtVariationPoint_X[j];
					 TRACE("coordinates x=%d,  coordinates y=%d\n", x,y);
				}
				TRACE("\n\n");
			}
		}
		TRACE("end of GlyphVariationData\n");
	}

}	// end of void CFontShowWnd::TraceVariationInf()
